//
//  DLLampControlSleepModeViewController.m
//  Zju_SmartHome
//
...
@interface DLLampControlSleepModeViewController ()<UINavigationControllerDelegate, UIImagePickerControllerDelegate, UIPopoverControllerDelegate>
...
- (void)viewDidLoad
{
  ...
  //self.tag=1;
    self.switchTag=1;
    NSString *r = [NSString stringWithFormat:@"%@",[[NSString alloc] initWithFormat:@"%1x",39]];
    NSString *g = [NSString stringWithFormat:@"%@",[[NSString alloc] initWithFormat:@"%1x",50]];
  ...
  if (fabs(([[UIScreen mainScreen] bounds].size.height - 568)) < 1){
    // 5 & 5s & 5c
    imgView.image = [UIImage imageNamed:@"sleeping_5"];
    ...
    // 6 & 6s
    imgView.image = [UIImage imageNamed:@"sleeping_6"];
    ...
    // 6p & 6sp
    imgView.image = [UIImage imageNamed:@"sleeping_6p"];
